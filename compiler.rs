...
impl Compiler {
    ...
    pub fn compile(&self) -> Result<String, CompilerError> {
        ...
        for stmt in &self.ast {
            ...
            match stmt {
                Stmt::Expr(expr) => {
                    let block = self.compile_expr(expr)?;
    ...
    fn compile_block(&self, ast: &BlockStmt) -> Result<String, CompilerError> {
    ...
    fn compile_expr(&self, expr: &Expr) -> Result<String, CompilerError> {
        let mut source = String::new();
        match expr {
            Expr::Let(ident, expr) => {
            ...
            Expr::Call { func, args } => {
                ...
                if !self.scope.borrow_mut().has_fn(function.into()) {
                    return Err(CompilerError::not_found());
                };
                source.push_str(function);
                source.push_str("(");
